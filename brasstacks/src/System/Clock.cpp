...
Clock::HRC::time_point Clock::_update_delta_tick = Clock::_start_time;
...
std::atomic<float> Clock::_update_delta = 0.0f;
...
void Clock::update_delta_tick() {
    _update_delta_tick = HRC::now();
}
...
void Clock::update_tock() {
    ...
    _update_delta = 1e-6f * static_cast<float>(
        duration_cast<microseconds>(HRC::now() - _update_delta_tick).count()
    );
}
